...
def main():
    ...
    utils.save_dataframe(df, MASTER_DATASET)
    ...
    box_office_df = df[(df["budget"] >= 100) & (df["revenue"] >= 100)].copy()

    # Save box office dataset

    utils.save_dataframe(box_office_df, BOX_OFFICE_DATASET)
    ...
    content_ratings_df = df[df["content_rating"].isin(valid_ratings)].copy()
    ...
    print(f"Excluded from box-office: {len(df) - len(box_office_df)} rows")

    print("\nBox-office validation:")

    print("Budget < 100 :", (box_office_df["budget"] < 100).sum())

    print("Revenue < 100:", (box_office_df["revenue"] < 100).sum())

    print("Missing budget :", box_office_df["budget"].isna().sum())

    print("Missing revenue:", box_office_df["revenue"].isna().sum())

    print("\nContent-rating classes: ")

    print(content_ratings_df["content_rating"].value_counts().to_string())
