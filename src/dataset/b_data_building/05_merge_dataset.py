...
TMDB_DATASET = TMDB_DIR / "tmdb_metadata_with_certi.csv"

MULTIMODEL_DATASET = BASE_DIR / "multimodel_dataset.csv"
...
def merge_datasets(
    ...
    imdb_df = imdb_df[["imdb_id", "release_year", "rating", "num_votes"]]
...
def main():
    ...
    utils.print_section("MERGING IMDb AND TMDB DATASET")

    # Load IMDb dataset

    imdb_df = pd.read_csv(IMDB_DATASET)

    print(f"IMDb movies : {len(imdb_df):,}")

    # Load TMDB metadata

    tmdb_df = pd.read_csv(TMDB_DATASET)
    ...
    merged = removing_missing_posters(merged)

    print(f"Movies without posters removed : {before - len(merged):,}")
    ...
    merged = removing_missing_poster_files(merged)

    print(f"Missing poster files removed : {before - len(merged):,}")
    ...
    utils.print_section("FINAL MULTIMODAL DATASET")

    print(f"Movies : {len(merged):,}")
